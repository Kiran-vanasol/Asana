...
class WorkoutViewModel: ObservableObject {
    ...
    init(poses: [APIPose] = [], workoutType: String = "BackPain", workoutId: String) {
        self.poses = poses
    ...
    func finishWorkout() {
        guard !isWorkoutFinished else { return }
             ...
             // record streak
             StreakService.shared.recordCompletion(for: workoutId) { result in
                 switch result {
                 case .success(let info):
                     print("Streak updated: \(info.streakCount)")
                 case .failure(let err):
                     print("Failed to update streak:", err.localizedDescription)
                 }
             }
        workoutCancellable?.cancel()
        introCancellable?.cancel()
